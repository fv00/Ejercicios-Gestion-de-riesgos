...
```{r}
pesos_knn <- modelos_knn$learn$X
plot(density(modelo_knn))
```
...
```{r}
sigDist1 <- sigest(Default ~ ., data=datos_modelo_final, frac = 1)
```

```{r}
svmTuneGrid <- data.frame(.sigma = sigDist1[1], .C = 2^(-2:7))
```


```{r}
y1 = as.factor(datos_modelo_final$Default)
levels(y1) = c("No","Yes")
```
...
```{r}
par(mfrow = c(2,2))
plot(density(d.hat), main = "Densidad Modelo Logid")
